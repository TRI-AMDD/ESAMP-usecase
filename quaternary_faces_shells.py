...
class ternaryfaces_shells:
    ...
    def toCart(self, quatcomps, skipinds=list(range(4)), nshell=0):#binary and ternary lines need to be plotted multiple times so returns  set of (inds,x,y)
...
#        y=numpy.empty(len(qc), dtype='float32')
    ...
    def scatter(self, quatcomps, c, skipinds=list(range(4)), s='patch', **kwargs):
        if s=='patch':
        ...
        for nshell in range(int(self.nint//4)+int(self.nint%4>0)):
            ...
            for inds, x, y in inds_x_y:
                if patchfcn is None:
                    ...
                    list(map(patchfcn, x, y, shellc[inds]))
    ...
    def get_plot_coords(self, quatcomps, skipinds=list(range(4))):
        quatcomps=numpy.int32(numpy.round(quatcomps*self.nint))
        nshell_inds_x_y=[]
        for nshell in range(int(self.nint//4)+int(self.nint%4>0)):
            ba=((quatcomps==nshell).sum(axis=1, dtype='int32')>0)&((quatcomps>=nshell).prod(axis=1, dtype='int32')>0)
    ...
    def quatscatter(self, quatcomps, c, skipinds=list(range(4)), azim=-60, elev=30, alphaall=.2, alphashell=1., fontsize=14, outline=True,  **kwargs):
        ...
        if self.nint%4==0: #single point with no frame
            ...
            if True in ba:
                ...
                if fontsize>0:
                    q.label(ha='center', va='center', fontsize=fontsize)
                q.set_projection(azim=azim, elev=elev)
    def quatplot3D(self, quatcomps, c, skipinds=list(range(4)), azim=-60, elev=30, alphaall=.2, alphashell=1., fontsize=14, outline=True,  **kwargs):
        numsubs=int(self.nint//4)+1
        quatcomps=numpy.int32(numpy.round(quatcomps*self.nint))
        for nshell in range(int(self.nint//4)+int(self.nint%4>0)):
            ba=((quatcomps==nshell).sum(axis=1, dtype='int32')>0)&((quatcomps>=nshell).prod(axis=1, dtype='int32')>0)
    ...
    def toComp(self, x, y, skipinds=list(range(4))):#takes a single x,y coord from the axes and gets the tirangle by trial and error and converts to a,b,c,d/ skipinds must be the same as that used in .scatter()
